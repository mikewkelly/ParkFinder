...
public class ParkFinder implements EntryPoint {
	...
	int INITIAL_ZOOM_LEVEL = 11;

	// the main panel
	DockLayoutPanel mainPanel = new DockLayoutPanel(Unit.PX);

	HorizontalPanel topPanel = new HorizontalPanel();
	VerticalPanel leftTopPanelContents = new VerticalPanel();
	VerticalPanel rightTopPanelContents = new VerticalPanel();
	...
	private final UserSearchServiceAsync searchService = GWT
			.create(UserSearchService.class);
				...
				new AsyncCallback<LoginInfo>() {
					...
					public void onSuccess(LoginInfo result) {
						...
						if (loginInfo.isLoggedIn()) {
							...
							Resources.INSTANCE.ClientParkFinder()
									.ensureInjected();
							loadParkTable();

							leftTopPanelContents.add(headerPanel);
							leftTopPanelContents.add(userStatus);
							userStatus.setText("Welcome, "
									+ loginInfo.getEmailAddress());
							leftTopPanelContents.add(signOutLink);
							signOutLink.setHref(loginInfo.getLogoutUrl());
							leftTopPanelContents.add(searchPanel);
							rightTopPanelContents.add(fbPanel);
							fbPanel.setVisible(true);
							topPanel.add(leftTopPanelContents);
							topPanel.add(rightTopPanelContents);
							mainPanel.addNorth(topPanel, 400);
	...
	public void loadLogin() {
		...
		leftTopPanelContents.add(loginLabel);
		leftTopPanelContents.add(signInLink);
		leftTopPanelContents.add(headerPanel);
		leftTopPanelContents.add(searchPanel);
		topPanel.add(leftTopPanelContents);
		rightTopPanelContents.add(fbPanel);
...
//TODO	// importData();
			...
			public void onSuccess(String search) {
				...
				previousSearchfac.addChangeHandler(new ChangeHandler() {
					public void onChange(ChangeEvent event) {
						...
						String theSearch = previousSearchfac
								.getValue(selectIndex);
						String newNbhd = theSearch.substring(
								theSearch.indexOf("_") + 1, theSearch.length());
						String newFacilityToSearch = theSearch.substring(0,
								theSearch.indexOf("_"));
						nbhd = newNbhd;
	...
	private void SearchFacility() {
		...
		facilityService.getFacility(facilityToSearch,
				new AsyncCallback<Facility[]>() {
					...
					public void onSuccess(Facility[] facilities) {
						...
						parkService.getParks(parklist,
								new AsyncCallback<Park[]>() {
									int row = 1;
	...
	private void getParkbyName() {
		...
		successMsg.setVisible(true);
		parkService.getParkByName(searchParkName, nbhd,
				new AsyncCallback<Park[]>() {
					int row = 1;
					...
					public void onSuccess(Park[] parks) {
						...
						if (!searchParkName.isEmpty()
							...
							if (!nbhd.equals("All"))
								successMsg.setText("Found parks with ' "
										+ searchParkName + " ' : " + --row
										+ " in " + nbhd + " Neighbourhood.");
							else
								successMsg.setText("Found parks with ' "
										+ searchParkName + " ' : " + --row
										+ " in Vancouver.");
	...
	private void showParkInTable(Park park, int row) {
			...
			public void onClick(ClickEvent event) {
				...
				areaService.getArea(parkId, new AsyncCallback<Area[]>() {
					...
					public void onSuccess(Area[] areas) {
						...
						successMsg.setText("There are " + areas.length
								+ " status notes.");
		...
		showFacilityButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				facilityService.getFacilitybyPark(parkId,
						new AsyncCallback<Facility[]>() {
							public void onFailure(Throwable error) {
								Window.alert("SORRY, SERVER NOT AVAILABLE");
							}
							...
							public void onSuccess(Facility[] facilities) {
								...
								successMsg.setText("There are "
										+ facilities.length + " status notes.");
	...
	private void updateMap(Park[] parks) {
		...
		// How to handle the user's selection of a specific park in the list
		// next to the map
						...
						if (selected != null) {
							for (Park p : parksFinalCopy) {
								if (p.getName().toUpperCase()
									...
									LatLong theLatLong = convertGMDtoLatLong(p
											.getGoogleMapDest());
									LatLng locationPoint = LatLng.newInstance(
											theLatLong.getLat(),
											theLatLong.getLong());
									...
									parkInfoFlexTable.setText(2, 0,
											neighbourhoodName);

									infoVerticalPanel.add(parkInfoFlexTable);
									theMap.getInfoWindow().open(
											theMap.getCenter(),
		...
		for (Park p : parks) {
			...
			final LatLng locationPoint = LatLng.newInstance(
					theLatLong.getLat(), theLatLong.getLong());
			...
			theMarker.addMarkerClickHandler(new MarkerClickHandler() {
				...
				public void onClick(MarkerClickEvent event) {
					...
					infoVerticalPanel.add(parkInfoFlexTable);
					theMap.getInfoWindow().open(locationPoint,
							new InfoWindowContent(infoVerticalPanel));

					// selected the park name in the list associated with the
					// clicked marker
					@SuppressWarnings("unchecked")
					SingleSelectionModel<String> selectionModel = (SingleSelectionModel<String>) mapParkList
							.getSelectionModel();
		...
		@SuppressWarnings("unchecked")
		SingleSelectionModel<String> listSelectionModel = (SingleSelectionModel<String>) mapParkList
				.getSelectionModel();
		for (Park park : parks) {
			if (park.getName().toUpperCase()
					.equals(searchParkName.toUpperCase())) {
